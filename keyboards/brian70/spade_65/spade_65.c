...
void eeconfig_confinfo_init(void) {
    ...
    if (!confinfo.flag || confinfo.devs > DEVS_2G4) {
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    ...
    if (rgbrec_is_started() && (!(keycode == RP_P0 || keycode == RP_P1 || keycode == RP_P2 || keycode == RP_END || keycode == RM_NEXT || keycode == MO(_FL) 
    || keycode == MO(_MFL) || keycode == MO(_FBL) || keycode == MO(_FMBL)))) {
...
bool process_record_kb(uint16_t keycode, keyrecord_t *record) {
    ...
    switch (keycode) {
        ...
        case LED_TOG:{
            if (record->event.pressed){
                confinfo.use1_flag = !confinfo.use1_flag;
                eeconfig_confinfo_update(confinfo.raw);
            }
            return false;
            break;
...
void housekeeping_task_user(void) {
    ...
    bat_full_flag = (gpio_read_pin(BAT_FULL_PIN) == BAT_FULL_STATE);
...
void bat_indicators(void) {
    ...
    if (charging_state && (bat_full_flag || *md_getp_bat() >= 100)) {
...
bool rgb_matrix_indicators_advanced_kb(uint8_t led_min, uint8_t led_max) {
    // LED 2 mirrors LED 1 - handled via rgb_matrix_set_color after user indicators
    
    // LED 2 mirrors LED 1 - ws2812_array removed in newer QMK
    ...
    if (((*md_getp_state() == MD_STATE_CONNECTED) || USB_DRIVER.state == USB_ACTIVE) && confinfo.use1_flag == true)
    {
        ...
        if (!keymap_is_base_layer())     
            rgb_matrix_set_color(HS_RGB_BLINK_INDEX_LCTL, RGB_WHITE);
    }
...
void keyboard_pre_init_kb(void) {
    // Force the physical USB connection open immediately on boot
