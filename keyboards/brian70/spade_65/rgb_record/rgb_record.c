...
void rgbrec_update_current_channel(uint8_t channel) {
    ...
    uint32_t offset = channel * sizeof(rgbrec_buffer);
...
void eeconfig_init_user_datablock(void) {
    eeconfig_update_user_datablock(rgbrec_default_effects, 0, sizeof(rgbrec_default_effects));
}
...
uint8_t record_color_read_data(void) {
    ...
    uint8_t hs_c = 0;
    uint32_t offset = sizeof(rgbrec_default_effects) + 4 + hs_mode;
    eeconfig_read_user_datablock(&hs_c, offset, sizeof(hs_c));
    if (hs_c > RGB_HSV_MAX) {
        return 0;
    }
    return hs_c;
...
void record_color_hsv(bool status) {
    ...
    uint32_t offset = sizeof(rgbrec_default_effects) + 4 + find_index();
    eeconfig_update_user_datablock(&rgb_hsv_index, offset, sizeof(rgb_hsv_index));
}
